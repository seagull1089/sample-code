...
/**
 * Created by aragipindi on 3/26/15.
 *
...
public class ZigZag {
    public static int getMaxZigZagLength(int[] numbers){
        int[] pes = new int[numbers.length];
        int[] nes = new int[numbers.length];
        ...
        for(int i = 0; i < numbers.length ; i++){
            ...
            nes[i] = 1;
            for(int j = 0; j < i ; j++){
                if(numbers[i] > numbers[j] && pes[i] < nes[j] +1){
                    pes[i] = nes[j] +1;
                    if(pes[i] > max){
                        max = pes[i];
                    }
                }else if(numbers[i] < numbers[j] && nes[i] < pes[j] +1){
                    nes[i] = pes[j] +1;
                    if(nes[i] > max){
                        max = nes[i];
                    }
    ...
    public static void main(String[] args){
        //System.out.println(getMaxZigZagLength(new int[]{1,7,4,9,2,5}));
        System.out.println(getMaxZigZagLength(new int[]{70, 55, 13, 2, 99, 2, 80, 80,
